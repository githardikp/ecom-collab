...
import { userZodSchema, addressZodSchema } from '../validators/user.validator'
import { User } from '../models/user.models'

export const registerUser = async (req, res) => {
    try {
        const validatedUser = userZodSchema.parse(req.body)
        const existingUser = User.findOne({ email: validatedUser.email })
        if (existingUser) return res.status(200).json({ message: "User already exists" })

        const hashedPassword = bcrypt.hash(validatedUser.password, 10)
        validatedUser.password = hashedPassword

        const user = User.create(validatedUser)
